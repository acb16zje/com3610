...
\usepackage[toc, acronym, automake, nomain]{glossaries}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \draw [arrow] (has_next_commit) -- node [anchor=east] {yes} (match_regex);
    \draw [arrow] (has_next_commit) -- node [anchor=south] {no} (output);
    \draw [arrow] (match_regex) -- (for_mods);
    \draw [arrow] (for_mods) -- (has_next_file);
    \draw [arrow] (has_next_file) -- node [anchor=east] {yes} (file_supported);
    \draw [arrow] (has_next_file) -| node [very near start, anchor=south] {no} (-5.5,-7.5) ++ (0,
    0) -- (has_next_commit);
    \draw [arrow] (file_supported) -- node [anchor=south] {yes} (search_diff);
    \draw [arrow] (file_supported) -| node [very near start, anchor=south] {no} (-5.5, -14.5) ++ (0,
    0) -- (has_next_file);
    \draw [arrow] (search_diff.north) |- (has_next_commit.east);
...
aspects to consider:
\begin{enumerate}
  \item \textbf{Completeness:} If the objective were to achieve high completeness, then the regular
  ...
  \item \textbf{Correctness:} If the objective were to achieve high correctness, then the regular
...
commit message does not always summarise the actual changes in the commit object. It is unable to
fully identify a vulnerability-fixing commit without analysing the code changes.
...
\begin{enumerate}
  ...
  \item The vulnerability patterns identified
  \item The vulnerable code changes identified
\end{enumerate}
