...
challenges of this project are data, misclassification, evaluation and time. The subsequent
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.75\linewidth]{images/code_diff.png}
...
\subsection{Structure}
\hyperref[fig:directory_structure]{\textbf{Figure \ref*{fig:directory_structure}}} below shows the
...
\verb|vulnerability.py| and \verb|language.py| contain the class \verb|Vulnerability| and
\verb|Language| respectively. It is not necessary to create an individual file for each programming
language in the \verb|languages| folder, but it is recommended in practice for better
...
\begin{figure}[H]
  ...
  \caption{An example of expected search for the query \textbf{`\acrshort{cve}'}.}
  \label{figure:keyword_search}
\end{figure}

\hyperref[figure:keyword_search]{\textbf{Figure \ref*{figure:keyword_search}}} shows an example of
expected search results returned for the query \textbf{`\acrshort{cve}'}.

\section{Vulnerable Code Searching}

\subsection{Flawfinder Integration} \label{subsec:flawfinder}
ff

\subsection{Bandit Integration} \label{subsec:bandit}
ff
...
extra programming language, an instance of the \verb|Language| class must be created. The
`constructor' (the \verb|__init__| method) takes three parameters, a rule set, a list of file
...
\begin{enumerate}
  ...
  search process, the tool is able to perform faster, and it is less likely to obtain false positive
  results.
\end{enumerate}

The tool will be word boundary \verb|\b|

With these justifications, it is predicted that the tool would not have better performance
than Flawfinder (\hyperref[subsec:flawfinder]{\textbf{Section \ref*{subsec:flawfinder}}}) or Bandit
(\hyperref[subsec:bandit]{\textbf{Section \ref*{subsec:bandit}}}).

\section{Storing the Results}
In Python, dictionary and \acrshort{json} are interchangeable
