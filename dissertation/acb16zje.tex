...
\begin{figure}[H]
  \centering
  \includegraphics[width=.85\linewidth]{images/regex_groups.png}
...
After commit message matching, the next step is to analyse the code changes in the commits. There
are three change types: added, deleted, and unchanged. The vulnerabilities identified in different
change types imply different information about the commit.
...
\begin{longtable}{|p{5.6cm}|p{5.6cm}|>{\columncolor[HTML]{B7E1CD}}c|}
  ...
  & Pass \\ \hline
  Given a matching string and a regular expression & The search operation returned the matched
  groups, and the matched groups are correct & Pass \\ \hline
  \caption{Test cases of commit message matching.}
\end{longtable}

\subsection{Vulnerable Code Searching}

\subsubsection*{Flawfinder (C/C++) and Bandit (Python)}
\begin{longtable}{|p{5.6cm}|p{5.6cm}|>{\columncolor[HTML]{B7E1CD}}c|}
  \hline \endfirsthead
  \rowcolor[HTML]{D8D8D8}
  \multicolumn{1}{|c|}{Test Case} & \multicolumn{1}{|c|}{Expected Result} & Status \\ \hline
...
An experiment is carried out to test the performance of the tool.
\begin{itemize}
  ...
  \item \textbf{Dependent variables:} The time taken to complete the analysis process
\end{itemize}

It has been observed that the time taken to complete the analysis process is negative correlation to
the number of commits to analyse and the size of each commit.
